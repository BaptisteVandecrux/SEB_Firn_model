function [L, LHF, SHF, theta_2m, q_2m , ws_10m, Re] ...
    ...
    Tsurf,theta , pres,rho_atm,  z_WS, z_T, z_RH, snowbkt, rho_surf, c)
...
% Update BV2017: z_0 is calculated once at the beginning
if snowthick > c.smallno
    ...
    if snowbkt > c.smallno
        ...
        z_0 = max(c.z0_old_snow, ...
            c.z0_old_snow + (c.z0_ice -c.z0_old_snow)*(rho_surf - 600)/(920 - 600));
    end
else
...
if theta >= Tsurf && WS >= c.WS_lim   % stable stratification
    ...
    ws_10m    =          u_star /c.kappa * (log(10/z_0) - psi_m2 + psi_m1);
        ...
        if L == 0 || (abs((L_prev - L) / L_prev) < c.L_dif)
            break
        end
    end
...
if theta < Tsurf && WS >= c.WS_lim   % unstable stratification
    ...
    ws_10m    =          u_star /c.kappa * (log(10/z_0) - psi_m2 + psi_m1);
        ...
        L    = u_star^2 * theta*(1 + ((1 - c.es)/c.es)*q) /...
            (c.g * c.kappa * th_star*(1 + ((1 - c.es)/c.es)*q_star));
        if abs((L_prev - L) / L_prev) < c.L_dif
            break
        end
