...
folderlist = {[WorkingFolder, '/','CP1_0_SiCL_pr0.001_Ck1.00_darcy_wh0.10'];...
    [WorkingFolder, '/','DYE-2_0_SiCL_pr0.001_Ck1.00_darcy_wh0.10'];...
    [WorkingFolder, '/','NASA-E_0_SiCL_pr0.001_Ck1.00_darcy_wh0.10'];...
    [WorkingFolder, '/','NASA-SE_0_SiCL_pr0.001_Ck1.00_darcy_wh0.10'];...
    [WorkingFolder, '/','NASA-U_0_SiCL_pr0.001_Ck1.00_darcy_wh0.10'];...
    [WorkingFolder, '/','Saddle_0_SiCL_pr0.001_Ck1.00_darcy_wh0.10'];...
    [WorkingFolder, '/','SouthDome_0_SiCL_pr0.001_Ck1.00_darcy_wh0.10'];...
    [WorkingFolder, '/','Summit_0_SiCL_pr0.001_Ck1.00_darcy_wh0.10'];...
    [WorkingFolder, '/','TUNU-N_0_SiCL_pr0.001_Ck1.00_darcy_wh0.10']};
    
station={'CP1','DYE-2','NASA-E',...
    'NASA-SE','NASA-U','Saddle',...
    'SouthDome','Summit','TUNU-N'};
station2 = station;
for i =1:length(station)
    station2{i} = strrep(station{i},'-','_');
    station2{i} = strrep(station2{i},' ','_');
end
...
for i = 1:length(folderlist)
    ...
    namefile = sprintf('%s/%s_surf-bin-%i.nc',folderlist{i},station{i},1);
...
%% loading station data
    c.InputAWSFile = ['C:\Data_save\2020 JoG data\Corrected\AWS data\',...
        station{i},'\data_',station{i},'_combined_hour.txt'];
...
%     delta_rho_melt = max(0,delta_rho_melt + delta_rho_rfrz);
        ...
        %% Surface variable
        namefilesurf = [WorkingFolder, '\data out/', station2{i}, '_surface.nc'];
        c.em=0.98;
        LRout_mdl_cor = c.em * c.sigma * (Tsurf+273.15).^4 + (1 - c.em) * LRin;
        time = time-1;
        [~, ~, ~, ~, ~, ~, ~,H_surf, ~, ~, ~, ~] ...
            = LoadTempModel(folderlist{i},station{i});
    ...
    figure
        ...
        hold on
        plot(meltflux)
        SEB = GF + SRin-SRout+LRin-LRout_mdl_cor+SHF+LHF;
        SEB(Tsurf<0)=0;
        SEB(SEB<0)=0;
        plot(SEB)
        axis tight square
...
%%
% path_orig = 'C:\Data_save\2020 JoG data\Corrected\data out\resource_map_doi_10_18739_A2V698C33\data';
% 
% for i = 1:length(folderlist)
%     disp(station{i})
%     % loading original data
%     namefile = sprintf('%s/%s_surface.nc',path_orig,station2{i});
%     finfo = ncinfo(namefile);
%     names={finfo.Variables.Name};
%     
%     namefile2 = sprintf('%s/%s_surf-bin-%i.nc',folderlist{i},station{i},1);
%     finfo = ncinfo(namefile);
%     for ii= 1:length(names)
%         figure
%         tmp = ncread(namefile,names{ii});
%         scatter(ncread(namefile,names{ii}),ncread(namefile,names{ii}))
%         hold on
